...
let request_previous_blocks state block =
  prerr_endline "request_previous_blocks";
  if
...
let rec try_to_apply_block state update_state block =
  ...
  if is_new_state_root_hash then (
    write_state_to_file
      ...
      prev_protocol;
    (* prerr_endline "We should call try_to_commit_state_hash:"; *)
...
let received_block state update_state block =
  prerr_endline
    (Printf.sprintf
       "************* received block %s of height %Ld for state of height %Ld"
       (Crypto.BLAKE2B.to_string block.Block.hash)
       block.Block.block_height state.Node.protocol.Protocol.block_height);
  let%ok () =
  ...
  let () =
    ...
    | Some (b, round) ->
      ...
      let signatures =
        let _ =
          prerr_endline
            (Printf.sprintf "*** Looking for signatures for hash %s"
               (Crypto.BLAKE2B.to_string previous_hash)) in
...
let received_precommit_block state update_state ~block ~sender ~hash
  ...
  let%assert () =
    ( `Invalid_signature_for_this_hash,
      Signature.verify ~signature:hash_signature block.Block.hash ) in
  let%assert () =
  ...
  | Some (block, _) ->
    ...
    (* let state =
       append_signature state update_state ~hash ~signature:hash_signature ~height ~round in *)
  ...
  | None ->
    ...
    prerr_endline "Consensus not reached!";
    Ok ()
...
let received_consensus_step state update_state operation sender hash
  ...
  let%await state, consensus =
    ...
    | ProposalOP (height', _, Block b, _) when height' > Int64.add height 1L ->
      ...
      prerr_endline
        (Printf.sprintf "I have now state height %Ld"
  ...
  | Ok () ->
    ...
    prerr_endline
      (Printf.sprintf "**** We're here with state of height %Ld"
         consensus.Tendermint.node_state.Node.protocol.Protocol.block_height);
    prerr_endline
    ...
    prerr_endline
      (Printf.sprintf "Consensus has %d subprocesses"
         (List.length consensus.Tendermint.procs));
    ...
    Lwt.return
      begin
        ...
        | PrecommitOP (_height, _round, Block b) ->
          ...
          prerr_endline
            (Printf.sprintf "Consensus has %d subprocesses and height is %Ld"
               (List.length consensus.Tendermint.procs)
               consensus.Tendermint.node_state.Node.protocol
          ...
          prerr_endline
            (Printf.sprintf "Consensus has %d subprocesses"
               (List.length consensus.Tendermint.procs));
          prerr_endline
            (Printf.sprintf "**** After consensus, state of height %Ld"
               consensus.Tendermint.node_state.Node.protocol
                 .Protocol.block_height);
          !set_consensus consensus;
          Ok ()
